...
const Wishlist = () => {
  ...
  let pass = localStorage.getItem("ID");
  if (!pass) {
    history.push("/login");
  }
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      for (let product of result.data) {
        const wishProduct = await axios.get(`/product/${product.productId}`);
        setWishlists((old) => [...old, wishProduct.data]);
      }
      //   result.data.forEach(async (product) => {
      //     //wishlists.push(product.productId);
      //     const wishProduct = await axios.get(`/product/${product.productId}`);
      //     setWishlists((old) => [...old, wishProduct.data]);
      //   });
    }
  ...
  wishlistarray.forEach((array) => {
    wishlistarrayid.push(array.wishlistId);
  });
  console.log(wishlistarrayid);
  ...
  async function deleteWish(wishlistId) {
    wishlistId.forEach(async (Id) => {
      console.log(Id);
      const result = await axios.delete(`/wishlist/${Id}`);
      //console.log(result.data);
      if (result.data === 0) {
        alert("刪除失敗");
        return;
  ...
  return (
    <>
      ...
      <Container>
          ...
          <NavRight2>
            ...
            <StyledLink to="/cart">
              <NavIcon>
                ...
                <CartCount>{cartId ? cartId.length : 0}</CartCount>
              </NavIcon>
            </StyledLink>
        ...
        <Main>
          <Carts>
            <NewTitle>Wishlist</NewTitle>
          </Carts>
          <CartContainer>
            <CartHeader>
              <CartTitle>My Favorites</CartTitle>
              <RemoveAll onClick={() => deleteWish(wishlistarrayid)}>
                Remove All
              </RemoveAll>
            </CartHeader>
            {wishlists.map((wishlist, index) => (
              <CartItem>
                <ProductImg
                  src={
                    "http://localhost:8080/productImg/" + wishlist.productImg
                  }
                ></ProductImg>
                <CartAbout>
                  <ProductName>{wishlist.productName}</ProductName>
                  <ProductInfo>${wishlist.productPrice}</ProductInfo>
                </CartAbout>
                <CartAbout>
                  <SurplusText>Surplus</SurplusText>
                  <ProductSurplus>{wishlist.productCount}</ProductSurplus>
                </CartAbout>
                <CartCounter>
                  <WishRemove
                    onClick={() => deleteWish([wishlistarrayid[index]])}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      data-name="Layer 1"
                  ...
                  <WishRemove onClick={() => addCart(wishlist.productId)}>
                    <svg
                      id="Layer_1"
                      data-name="Layer 1"
                      xmlns="http://www.w3.org/2000/svg"
            ...
            <CartLine>
              <Checkout>All add to cart</Checkout>
            </CartLine>
          </CartContainer>
        </Main>
        <Footer>
