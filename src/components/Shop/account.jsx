...
const Account = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const result = await axios.post("/customer/account", id, {
        headers: { "Content-Type": "text/plain" },
      });
  ...
  async function customerUpdate() {
    ...
    const result = await axios.put("/customer/update", auth);
    if (result.data !== 0) {
      alert("修改成功");
  ...
  async function customerDelete() {
    await axios.delete(`/wishlist/customer/${id}`);
  ...
  return (
    <>
      ...
      <Container>
        ...
        <Main>
          <Carts>
            <NewTitle>Account</NewTitle>
          </Carts>
          <CartContainer>
            <CartHeader>
              <CartTitle>My Account</CartTitle>
              <RemoveAll onClick={logout}>Logout</RemoveAll>
              <RemoveAll onClick={customerDelete}>Delete Account</RemoveAll>
            </CartHeader>
            <CartItem>
              <CartAbout>
                <ProductName>{account.customerName}</ProductName>
                <ProductInfo>{account.customerEmail}</ProductInfo>
              </CartAbout>
              <CartAbout>
                <SurplusText>NewName</SurplusText>
                <ChangeInput id="newName"></ChangeInput>
              </CartAbout>
              <CartAbout>
                <SurplusText>NewEmail</SurplusText>
                <ChangeInput id="newEmail"></ChangeInput>
              </CartAbout>
              <CartAbout>
                <SurplusText>NewPassword</SurplusText>
                <ChangeInput id="newPassword"></ChangeInput>
              </CartAbout>
            </CartItem>
            <CartLine>
              <Checkout onClick={customerUpdate}>Changed</Checkout>
            </CartLine>
          </CartContainer>
        </Main>
        <Footer>
